...
import { SafeAreaProvider as ZynkSafeProv } from 'react-native-safe-area-context';
import ProductScreen from './NoveBiltFriendsGameSrc/IltienPageens/ProductScreen';
import QwopMainScreen from './NoveBiltFriendsGameSrc/IltienPageens/BilrapWarpers';

import { NavigationContainer as QwexNavCont } from '@react-navigation/native';
import { createNativeStackNavigator as FrintStack } from '@react-navigation/native-stack';

import ZoqtOnboard from './NoveBiltFriendsGameSrc/IltienPageens/EvoninOnbrobordnig';
import { GestureHandlerRootView as VynkGestureRoot } from 'react-native-gesture-handler';

import QworpBoot from './NoveBiltFriendsGameSrc/IltienPageens/LenovLodAdingam';
// libs
import AsyncStorage from '@react-native-async-storage/async-storage';
import { LogLevel, OneSignal } from 'react-native-onesignal';
import AppleAdsAttribution from '@vladikstyle/react-native-apple-ads-attribution';
import DeviceInfo from 'react-native-device-info';
const XytrStack = FrintStack();

const QwexNavRoot = () => {
  const QwexScreen = XytrStack.Screen;
  const QwexNavigator = XytrStack.Navigator;

  const QwopTransitions = {
    fadeA: { headerShown: false, animation: 'fade' },
    fadeB: { headerShown: false, animation: 'fade' },
  };
  ///////////////////////////////////////////////////////////////////////
  ...
  const INITIAL_URL = `https://fresh-zone-lab.site/`;
  const URL_IDENTIFAIRE = `M0rgVLzT`;
  ...
  OneSignal.initialize('ac13835f-0ae8-4154-b521-aff3e0f8d5d9');
  ...
  const Route = ({ isFatch }) => {
    ...
    return (
      <QwexNavigator
        screenOptions={{
          contentStyle: { backgroundColor: '#0e0f14' },
          animationEnabled: true,
        }}
        initialRouteName="OnboardRhythmaFlow"
      >
        <QwexScreen
          component={ZoqtOnboard}
          options={{
            ...QwopTransitions.fadeB,
          }}
          name="OnboardRhythmaFlow"
        />

        <QwexScreen
          component={QwopMainScreen}
          options={{
            gestureEnabled: false,
            ...QwopTransitions.fadeA,
          }}
          name="BilrapWarpers"
        />
      </QwexNavigator>
  ...
  return (
    <QwexNavCont>
      {!isLoading ? <QworpBoot /> : <Route isFatch={route} />}
    </QwexNavCont>
  );
};
...
const ZySafeRoot = () => {
  return (
    <VynkGestureRoot style={{ flex: 1, backgroundColor: '#17104E' }}>
      <ZynkSafeProv>
        <QwexNavRoot />
      </ZynkSafeProv>
    </VynkGestureRoot>
